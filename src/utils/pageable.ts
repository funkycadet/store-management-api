...
const processOrderBy = (orderBys: string[] | string) => {
  ...
  if (!Array.isArray(orderBys)) {
    ...
    order[field.trim()] = direction.trim() === 'desc' ? Prisma.SortOrder.desc : Prisma.SortOrder.asc;
    return order;
  }
  orderBys && orderBys.forEach(orderBy => {
...
const responseToPageable = <T>(query: ProcessedPageable<T>, count?: number, data?: T[]) => {
